...
def main():
    wandb.init(project="DLops-Assignment-5", name="detector_pgd")
    ...
    x_train_adv = pgd_attack.generate(x=x_train)
    ...
    for epoch in range(1, 21):
        ...
        wandb.log({
            "epoch": epoch,
            "pgd_train_loss": train_loss,
            "pgd_train_accuracy": train_acc,
            "pgd_detection_accuracy": test_acc
        })

        if test_acc > best_acc:
            best_acc = test_acc
            torch.save(detector.state_dict(), "detector_pgd.pth")
            print(f"  ✅ Best detector saved! Detection Acc: {best_acc:.2f}%")

    print(f"\n🎉 PGD Detector Done! Best Detection Accuracy: {best_acc:.2f}%")
